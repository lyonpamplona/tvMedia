...
def scope_can_access(scope: Scope, company_id: int | None) -> bool:
    """Indica se o escopo atual pode acessar um registro da empresa informada.

    * Super admin sem empresa selecionada (``scope.company_id is None``): acessa
      qualquer empresa.
    * Demais casos (usuário comum, ou super admin personificando uma empresa):
      só acessa registros cuja ``company_id`` corresponde à empresa em foco.

    Usado nos endpoints de leitura/alteração por ID para impedir que um usuário
    de uma empresa atinja, via ID direto na URL, registros de outra empresa.
    """
