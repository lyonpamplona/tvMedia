...
def _get_owned_zone(db: Session, screen_id: int, zone_id: int) -> models.Zone:
    """Recupera uma zona garantindo que pertence à tela informada.

    Raises:
        HTTPException: 404 se a zona não existir ou não for da tela.
    """
...
def _validate_playlist(db: Session, playlist_id: int | None) -> None:
    """Valida que a playlist informada existe (quando informada)."""
...
async def create_zone(
    screen_id: int, data: schemas.ZoneCreate, db: Session = Depends(get_db)
) -> models.Zone:
    ...
    _validate_playlist(db, data.default_playlist_id)
...
async def update_zone(
    ...
    zone = _get_owned_zone(db, screen_id, zone_id)
    _validate_playlist(db, data.default_playlist_id)
...
async def delete_zone(
    screen_id: int, zone_id: int, db: Session = Depends(get_db)
) -> None:
