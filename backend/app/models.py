...
class Overlay(Base):
    ...
    name: Mapped[str] = mapped_column(String(255), nullable=False, default="Overlay")
    # Tipo de widget reutilizado do player: clock, weather, news, promo, text, html.
    kind: Mapped[str] = mapped_column(String(16), nullable=False, default="clock")
    # Config JSON (mesmo formato dos widgets) ou texto/HTML.
    content: Mapped[str | None] = mapped_column(Text, nullable=True)
    # Posicao na tela.
    position: Mapped[str] = mapped_column(String(16), nullable=False, default="bottom")
    # Largura/altura em % da tela (0 = automatico conforme a posicao).
    ...
    # Modo: "fixed" (sempre visivel) ou "timed" (aparece a cada intervalo).
    mode: Mapped[str] = mapped_column(String(8), nullable=False, default="fixed")
    interval_seconds: Mapped[int] = mapped_column(Integer, nullable=False, default=300)
    visible_seconds: Mapped[int] = mapped_column(Integer, nullable=False, default=15)
